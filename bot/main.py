...
{projects_status}
"""
    claude_prompt = f"""你是業老闆（樊業萱）的商業顧問，她是傳產數位轉型 + AI 落地顧問，目標是一人公司年純收 250 萬。
...
"""
    ...
    # 週學習合成：用 Claude 把累積的 learnings 更新回 background.md
    try:
        learnings, _ = gh_read_file("context/learnings.md")
        background, _ = gh_read_file("identity/background.md")
        if learnings and len(learnings) > 100:
            synthesis_prompt = f"""根據以下對話學習記錄，更新業主背景檔案，讓它更完整、更準確。
保留原有結構，只補充或修正有新資訊的部分。直接輸出更新後的完整 Markdown，不要解釋。
...
{learnings[-2000:]}
"""
            updated = claude_analyze(synthesis_prompt) if ANTHROPIC_API_KEY else gemini_text(synthesis_prompt)
            gh_write_file("identity/background.md", updated, f"learn: 週學習合成 {today}")
            logger.info("✅ 業主背景已更新")
    except Exception as e:
        logger.warning(f"週學習合成失敗: {e}")
...
def learn_from_conversation(text: str) -> None:
    """從對話中提取業主資訊，累積到 context/learnings.md"""
    if len(text.strip()) < 15:
        return
    background, _ = gh_read_file("identity/background.md")
    prompt = f"""
你是一個業主側寫助手。
現有業主背景：{background[:300]}
...
只輸出 JSON（嚴格格式）：
{{"has_insight": true或false, "insight": "一句話（15字內）"}}
"""
    try:
        result = re.sub(r"```json|```", "", gemini_text(prompt)).strip()
        data = json.loads(result)
        if data.get("has_insight") and data.get("insight"):
            insight = data["insight"].strip()
            current, _ = gh_read_file("context/learnings.md")
            if not current:
                current = "# 對話學習記錄\n\n"
            gh_write_file(
                "context/learnings.md",
                current + f"- {today_str()}：{insight}\n",
                f"learn: {insight[:30]}"
            )
    except Exception as e:
        logger.debug(f"learn_from_conversation: {e}")
...
def execute_vault_query(question: str) -> str:
    ...
    prompt = f"""你是小獅，業老闆的 AI 工作夥伴。
以下是 LionBrain 知識庫的內容：
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            learn_from_conversation(transcribed)
        except Exception:
            pass
    except Exception as e:
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if caption:
            try:
                learn_from_conversation(caption)
            except Exception:
                pass
    except Exception as e:
